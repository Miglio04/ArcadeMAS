...
public class Tools {
    ...
    public static JSONObject genJSONSchema( List<Map<String, Term>> examples ) {
        ...
        // BEGIN patch
        String functor = examples.get( 0 ).get( "functor" ).toString();
        // END patch
        ...
        for ( String key : examples.get( 0 ).keySet() ) {
            ...
            // BEGIN patch
            Set<JSONObject> types = new LinkedHashSet<>();
            ...
            // BEGIN patch
            boolean numericLike = hints.stream().anyMatch( Tools::looksLikeNumericPlaceholder )
                || looksLikeNumericPlaceholder( key )
                || looksLikeNumericPlaceholder( functor );
            ...
            // BEGIN patch
            if ( numericLike ) {
                String description = "This field contains a numeric value for " + functor + ".";
                if ( !hints.isEmpty() ) {
                    description += " The source placeholder is " + String.join( " or ", hints ) + ".";
                }
                description += " Use the number explicitly mentioned in the sentence.";
    ...
    // BEGIN patch
    private static boolean looksLikeNumericPlaceholder( String value ) {
        if ( value == null )
            return false;
        String normalized = value.trim().toLowerCase().replaceAll( "[^a-z0-9]", "" );
        return normalized.matches(
            "^(qt|qty|qtd|quant|quantity|count|num|number|amount|amounts|n|"
            + "budget|saldo|credit|credits|money|cash|cost|price|"
            + "maxcost|maxprice|spend|limit)$"
        );
    }
    // END patch
